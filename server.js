...
app.get('/api/:action(approve|reject)-leave', requireLogin, async (req, res) => {
    ...
    try {
        ...
        if (action === 'approve' && currentStatus === 'Pending') {
            ...
            await updateMasterCalendar(applicantUsername, leaveType, startDate, endDate, retrievedAppID);
            
            // If this is a WFH application, also update WFH count
            if (leaveType === 'WFH') {
                const wfhColumnIndex = 36; // WFH count column (AK)
                const currentWFHCount = parseFloat(userDataResponse.data.values[userRowIndex][wfhColumnIndex]) || 0;
                const newWFHCount = currentWFHCount + leaveDays;
                
                await sheets.spreadsheets.values.update({
                    spreadsheetId: SPREADSHEET_ID,
                    range: `${SHEET_NAME}!AK${userRowIndex + 1}`,
                    valueInputOption: 'USER_ENTERED',
                    requestBody: { values: [[newWFHCount]] }
                });
                
                console.log(`Updated WFH count for ${applicantUsername}: ${currentWFHCount} -> ${newWFHCount}`);
            }
        }
...
// Get master calendar data
app.get('/api/master-calendar', requireLogin, async (req, res) => {
    try {
        ...
        if (leaveApplicationsResponse.data.values && leaveApplicationsResponse.data.values.length > 1) {
            ...
            const approvedApplications = leaveApplicationsResponse.data.values.slice(1)
                .filter(row => row[7] === 'Approved');
            
            // Convert each approved application to calendar entries
            for (const app of approvedApplications) {
                const [id, username, leaveType, startDate, endDate, days, reason, status] = app;
                
                // Generate calendar entries for each day in the range
                const start = new Date(startDate);
                const end = new Date(endDate);
                
                for (let date = new Date(start); date <= end; date.setDate(date.getDate() + 1)) {
                    // Skip weekends for non-WFH leave types
                    if (leaveType !== 'WFH' && (date.getDay() === 0 || date.getDay() === 6)) {
                        continue;
                    }
        ...
        // Also check if there's a Master Calendar sheet and merge data
        try {
            const masterCalResponse = await sheets.spreadsheets.values.get({
                spreadsheetId: SPREADSHEET_ID,
                range: 'Master Calendar!A:C',
            });
            
            if (masterCalResponse.data.values && masterCalResponse.data.values.length > 1) {
                // Add existing master calendar entries
                masterCalResponse.data.values.slice(1).forEach(row => {
            ...
            console.log('Master Calendar sheet not found or error reading it');
        }
...
// Update Master Calendar when leave is approved
async function updateMasterCalendar(username, leaveType, startDate, endDate, applicationId) {
    try {
        // Check if Master Calendar sheet exists
        const sheetMetadata = await sheets.spreadsheets.get({
            spreadsheetId: SPREADSHEET_ID,
        });
        
        const masterCalendarExists = sheetMetadata.data.sheets.some(
            sheet => sheet.properties.title === 'Master Calendar'
        );
        
        if (!masterCalendarExists) {
            // Create Master Calendar sheet if it doesn't exist
            ...
            await sheets.spreadsheets.values.update({
                spreadsheetId: SPREADSHEET_ID,
                range: 'Master Calendar!A1',
                valueInputOption: 'RAW',
                requestBody: {
                    values: [['Date', 'User', 'Type', 'Application ID']]
                }
            });
        }
        ...
        console.error('Error updating master calendar:', error);
